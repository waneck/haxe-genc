...
let initialize_constructor con c cf =
	...
	| Some ({eexpr = TFunction tf} as e) ->
		...
		let el_vt = try
			let cf_vt = PMap.find (Expr.mk_runtime_prefix "vtable") c.cl_fields in
		...
		let cf_ctor = Expr.mk_class_field (Expr.mk_runtime_prefix "ctor") (TFun((v_this.v_name,false,v_this.v_type) :: args,con.com.basic.tvoid)) false p (Method MethNormal) [] in
